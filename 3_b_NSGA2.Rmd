...
source_python("D:/04_PROJECTS/2001_WIND_OPTIM/NSGA3/NSGA3_fin.py")
cur_dat<-Sys.Date()
...
for(n in 1:length(file.names)){
  ...
  a<-optim(as.integer(20),as.integer(100), as.character(tmp_path))
  par_rob<-as.data.frame(a[1])
  last_rob<-as.data.frame(a[2])
  par_fit<-as.data.frame(a[3])
  write.csv(par_fit,paste(out_path,paste("par_fitness",paste(tmp_name,"csv",sep="."),sep="/"),sep="/"))
  
  track<-as.data.frame(a[4])
  write.csv(par_fit,paste(out_path,paste("logbook",paste(tmp_name,"csv",sep="."),sep="/"),sep="/"))
  
  #graph of the three goals from the data logger
  WT<-ggplot(data=track, aes(x=GENERATION, y=N_WT, group=1)) +
  geom_line()
  
  CLUS<-ggplot(data=track, aes(x=GENERATION, y=CLUS, group=1)) +
  geom_line()
   
  ENER<-ggplot(data=track, aes(x=GENERATION, y=ENERDENS, group=1)) +
  geom_line()
  
  graph_path<-paste(out_path,"log_graphs",sep="/")
  tmp_plt<-ggarrange(WT,CLUS,ENER,3,1,c("min_NWT","max_clus","max_enerdens"))
  jpeg(file=paste(graph_path,paste(tmp_name, ".jpg",sep=""),sep="/"))
plot(tmp_plt)
dev.off()
  
  
  #join the data to the spatial pts
  df<-left_join(tmp_pts@data, par_rob, by=c('WT_ID'='WT_ID2'))
  df<-left_join(df, last_rob, by=c('WT_ID'='WT_ID2'))
  tmp_pts@data<-df
