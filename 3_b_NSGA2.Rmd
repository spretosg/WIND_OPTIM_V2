...
source_python("D:/04_PROJECTS/2001_WIND_OPTIM/NSGA3/NSGA_3_new.py")
cur_dat<-Sys.Date()
...
for(n in 1:length(file.names)){
  ...
  a<-optim(as.integer(MU),as.integer(NGEN), as.character(tmp_path))
  ...
  graph_path<-paste(out_path,"HV_graphs",sep="/")
  ...
  ###extract the logbook
  write.csv(log,paste(out_path,paste("logbook",paste(tmp_name,"csv",sep="."),sep="/"),sep="/"))
