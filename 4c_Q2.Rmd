...
###localhost
pts<-st_read("D:/04_PROJECTS/2001_WIND_OPTIM/WIND_OPTIM_git/intermediate_steps/4_wp/pts_selection/B4_FOR-.shp")
...
map_tmp<-ggplot(data = bound) +
    geom_sf() +
    geom_density_2d_filled(mapping = aes(x=st_coordinates(rob_pts)[,1] ,y=st_coordinates(rob_pts)[,2], fill= ..level.., alpha= ..level..), data=rob_pts, contour_var = "ndensity")+
  ...
  scale_fill_manual(limits = c("0.5","0.7","0.9"), values = c("red","green","blue"))+
    geom_sf(data = rob_pts, color= "black", size=0.8)+
     geom_sf(data = bound, fill = NA, color = "gray")
