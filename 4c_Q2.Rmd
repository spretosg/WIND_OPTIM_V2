...
tmp_dat<-subset(pareto,pareto$GROUP == "A" | pareto$GROUP == "REF"| pareto$GROUP == "C")

tmp_dat<-subset(pareto,pareto$GROUP == "B" | pareto$GROUP == "REF"| pareto$GROUP == "C")
...
geom_smooth(method=lm, se=FALSE, aes(color=SCEN, linetype = SCEN))+
  ...
  scale_linetype_manual(values=c("longdash", "longdash","longdash","longdash","solid"))+
  geom_point(shape = 4, alpha = 0.1, size =0.1, color = "grey40")+
    labs(x ="energy density [MWh/yha]",y="min <-- N_WT")+
 theme_light()+
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18))
...
jpeg(file=paste("D:/04_PROJECTS/2001_WIND_OPTIM/WIND_OPTIM_git/intermediate_steps/4_wp/descriptives/",paste(cur_dat,"trade-off.jpg",sep=""),sep=""),width = 450, height = 1200)
plot(plot_tmp)
dev.off()
#plot(plot_tmp)
