...
###localhost
pts<-st_read("Y:/people/spreto/2001_WIND_OPTIM/spat_pts_201204/B4.shp")
...
sum(subset(rob_pts,rob_pts$FFF==0 & rob_pts$DIST_FOR>1 & rob_pts$DIST_ISOS>1000)$prod_MW)
...
```{r}
tmpA<-rob_pts
tmpB<-st_read("Y:/people/spreto/2001_WIND_OPTIM/spat_pts_201204/B4_FFF-.shp")
...
joined<-subset(joined,joined$rob_parB3>0 & joined$FFF.x==1)
...
ggplot(data = bound) +
    geom_sf() +
    geom_sf(data = bound, fill = NA, color = "gray")+
    geom_sf(data = joined, aes(color= prod_MW.x), size=0.8)
