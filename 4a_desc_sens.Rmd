...
```{r}
cur_dat<-Sys.Date()
pareto<-read.csv("D:/04_PROJECTS/2001_WIND_OPTIM/WIND_OPTIM_git/intermediate_steps/3_wp/NSGA3_RES/out/fitness_all_201228.csv", sep=",")
#pareto<-read.csv("Y:/people/spreto/2001_WIND_OPTIM/par_fitness_all_201130.csv", sep=",")
#pareto<-subset(pareto,pareto$MODEL_RUN==A | pareto$MODEL_RUN==B)
...
mod_names<-levels(pareto$MODEL_RUN)
# restrictive scen
...
summary(lm(ENERDENS~MODEL_RUN+POLICY+RESTRICTION,pareto))
summary(lm(N_WT~MODEL_RUN+POLICY+RESTRICTION,pareto))
summary(lm(CLUS~MODEL_RUN+POLICY+RESTRICTION,pareto))
...
pareto$uni_scen<-paste(pareto$SCENARIO,pareto$MODEL_RUN,sep = "_")
