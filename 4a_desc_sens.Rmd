...
summary(lm(ENERDENS~MODEL_RUN+SCENARIO,pareto))
summary(lm(N_WT~MODEL_RUN+SCENARIO,pareto))
summary(lm(CLUS~MODEL_RUN+SCENARIO,pareto))



wilcox.test(tmp$CLUS ~ tmp$MODEL_RUN)
#graphical check
ggplot(tmp_par, aes(x=as.factor(MODEL_RUN), y=CLUS)) +   
    geom_boxplot()
ggplot(tmp_par, aes(x=as.factor(MODEL_RUN), y=N_WT)) +   
    geom_boxplot()
ggplot(tmp_par, aes(x=as.factor(MODEL_RUN), y=ENERDENS)) +   
    geom_boxplot()
