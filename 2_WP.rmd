...
BEV<-raster("D:/04_PROJECTS/2001_WIND_OPTIM/in/BEVÖLKERUNG/bev_200.tif")
BEV[is.na(BEV[])] <- 0
...
st_write(test1,"D:/04_PROJECTS/2001_WIND_OPTIM/WIND_OPTIM_git/viewshed_analysis", "view_inpoints1",driver = "ESRI shapefile",overwrite=T)
```
...
inpoints='D:/04_PROJECTS/2001_WIND_OPTIM/WIND_OPTIM_git/viewshed_analysis/View_inpoints1.shp'
...
cen$VIS_HA<-cen$VIS_AREA*4
cen$ENER_DENS_MWha<-cen$prod_MW/cen$VIS_HA
cen$EXPL<-cen$dist_str+cen$dist_PL

tmp_cen<-as(cen,"Spatial")
pgInsert(con,c("WT_PU_HEX","WT_pts_WP2_200831"),tmp_cen)

require(reshape2)
#wide to long for all distance measures
df_cen<-as.data.frame(cen)
melt.pts<-melt(df_cen[c(4,7:25,27:31,33:35)])
...
```
##Map two of the optimization goals
Here we map the distribution of the wind energy per visible HA of the turbine and the exploitation index.
```{r}
