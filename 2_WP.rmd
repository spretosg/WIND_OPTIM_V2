...
st_write(obj = cen, dsn = con, Id(schema="WT_PU_HEX", table="CEN_FIN_200916"))
...
for(i in 1:nrow(cen)){
  ...
  cen$count_rest[i]<- (cen$in_bln[i]+ cen$in_forest[i]+ cen$in_fff[i]+  cen$in_biosph[i]+   cen$in_birds[i]+cen$in_drymead[i]+cen$isos_buffe[i]+ cen$isos_kern[i]+ cen$in_deer[i]+ cen$unesco_buf[i]+ cen$unesco_cor[i])
  
  if(cen$DIST_BLN[i]>1 | cen$DIST_UNE_N[i]>1| cen$DIST_UNE_K[i]>1 |  cen$DIST_MEAD[i]>1 | cen$in_birds[i]==1| cen$in_drymead[i]==1 | cen$isos_buffe[i]==1| cen$isos_kern[i]==1|cen$in_deer[i]==1|cen$unesco_buf[i]==1|cen$unesco_cor[i]==1)
    cen$other[i]<-0
  else
    cen$other[i]<-1
}
